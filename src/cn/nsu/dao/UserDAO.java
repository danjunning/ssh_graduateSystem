...
public class UserDAO {
	...
	public User findById(java.lang.Integer id) {
		...
		try {
			User instance = (User) getCurrentSession().get("cn.nsu.entity.User", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public int insertUser(String uname,String password){
		...
		SQLQuery query = getCurrentSession().createSQLQuery(sql).addEntity(User.class);
	...
	public int updateUser(int userId,User user){
		String sql="update user u set u.uname=?,u.sex=?,u.birthday=?,u.email=?,u.mobile=?,u.address=? where u.userId=?"; 
		SQLQuery query = getCurrentSession().createSQLQuery(sql).addEntity(User.class);
	...
	public int updatePsw(int userId,String password){
		String sql="update user u set u.password=? where u.userId=?"; 
		SQLQuery query = getCurrentSession().createSQLQuery(sql).addEntity(User.class);
	...
	public boolean UserEquals(String uname,String password) {	
		String sql="select * from user u where u.uname=? and u.password=?";
		Query query = getCurrentSession().createSQLQuery(sql);
		query.setParameter(0, uname);
		query.setParameter(1, password);
		if(query.list().size()>0){
