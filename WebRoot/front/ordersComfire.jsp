...
<%	User user=(User)request.getSession().getAttribute("user");
    ArrayList<HashMap> preorderlist=(ArrayList<HashMap>)request.getSession().getAttribute("preorderlist");
...
<div class="web-container">
    ...
    <div class="order-prd-wrap">
        ...
        <hr>
        <div class="order-prd">
        <form action="placeOrders" id="ordered">
        <input type="hidden" name="address" id="param_address" value="">
        <%for(int i=0;i<preorderlist.size();i++){ %>
            <div class="or-single" style="">
                <div class="or-header">
                 &nbsp; &nbsp; &nbsp; <span class="sp-num">订单号： <%=preorderlist.get(i).get("orderNumber") %></span>
                </div><input type="hidden" name="shopcartId" value="<%=preorderlist.get(i).get("shopcartId")%>">
                <div class="or-img">
                    <a href=""  title="<%=preorderlist.get(i).get("productName") %>" target="_blank">
                        <img src="<%=preorderlist.get(i).get("productImage")%>">
                    </a>
                </div>
                <div class="or-name">
                    <h2 ><a class="link-detail"><%=preorderlist.get(i).get("productName") %></a></h2>
                    <span class="ui-number">颜色：<%=preorderlist.get(i).get("productColor") %>&nbsp;分类：<%=preorderlist.get(i).get("productStyle") %></span>
                </div>
                <div class="or-type">
                    <br><span>数量：<%=preorderlist.get(i).get("productQuantity") %></span><br>
                </div>
                <div class="or-price">
                    <br><p class="count-price">单价： ￥<%=preorderlist.get(i).get("productPrice") %></p>
                </div>
                <div class="or-detail">
                    <br><a href="">订单详情</a>
                </div>
                <div class="or-operate">
                    <br><p class="count-price">小计： ￥<span class="total"><%=preorderlist.get(i).get("totalPrice")%></span></p>
                </div>
            </div>
           <% }%> 
            </form>
    ...
    <div class="submit-wrap">
        <div class="sub-content">
            ...
            <div class="sub-button" >
                <button class="order-sub" id="order_submit">提交订单</button><span id="result"></span>
            </div>
        </div>
        <hr>
    </div>
...
<script>
    ...
    $(document).ready(function(){
    	var TotalPrice=0;
	    $('.or-operate .total').each(function(){
    	...
    	$("#order_submit").click(function(){
			...
			//alert($('input[type=hidden][name=address]').val());
			//alert($('input[type=hidden][name=shopcartId]').val());
